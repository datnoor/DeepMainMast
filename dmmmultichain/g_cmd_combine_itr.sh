...
if [ ! -e $OUTF/comb.st ];then
	...
	if [ ! -e $comb_model1 ];then
		python3 $PG $mtxfile1 --Nstock 10000 --OutPath $comb_model1 --SecAssemble $TIME --Ncpu 4
	fi
	if [ ! -e $comb_model2 ];then
		python3 $PG $mtxfile2 --Nstock 10000 --OutPath $comb_model2 --SecAssemble $TIME --Ncpu 4
	fi
	if [ -e $OUTF/VESPER_MODELs ];then
		if [ ! -e $comb_model3 ];then
			python3 $PG $mtxfile3 --Nstock 10000 --OutPath $comb_model3 --SecAssemble $TIME --Ncpu 4
		fi
		if [ ! -e $comb_model4 ];then
			python3 $PG $mtxfile4 --Nstock 10000 --OutPath $comb_model4 --SecAssemble $TIME --Ncpu 4
		fi
	else
		echo "Running without AF model output"
	fi
